#!/bin/bash
# =============================================================================
# watchclaw-critical-issue.sh — WatchClaw GitHub Critical Issue Generator
# =============================================================================
# Creates a GitHub issue when SECURITY STATUS is CRITICAL.
# Rate limit: 1 issue per 6 hours (persisted in ~/.watchclaw/watchclaw-state.json).
# Also called via: watchclaw issue critical --run-now [--force]
# =============================================================================
...
# ── Rate limit check ─────────────────────────────────────────────────────────
RATE_OK=$(python3 - "$WATCHCLAW_STATE" "$ISSUE_RATE_HRS" <<'PYEOF'
...
if [ "$RATE_OK" = "rate_limited" ] && [ "$FORCE" = "false" ]; then
    LAST=$(jq -r '.last_issue_at // "unknown"' "$WATCHCLAW_STATE" 2>/dev/null || echo "unknown")
...
# Title
title = f"[WatchClaw CRITICAL] Security Alert — {now_iso[:10]}"

# Body (Markdown)
body_lines = []
body_lines.append(f"## 🚨 WatchClaw SECURITY ALERT — CRITICAL STATUS")
body_lines.append(f"")
...
if clusters:
    ...
    body_lines.append(f"")
    for c in clusters[:3]:
        body_lines.append(f"- **{c['asn']}**: {c.get('unique_ips_7d','?')} IPs, {c.get('hostile_count','?')} hostile — `watchclaw enforce ban-asn {c['asn']} --mode ufw`")
    body_lines.append(f"")
...
body_lines.append(f"")
body_lines.append(f"- WatchClaw auto-ban enforcement active (UFW + fail2ban)")
...
body_lines.append(f"")
body_lines.append(f"- [ ] Review top offenders and verify bans: `watchclaw top --window 24h`")
body_lines.append(f"- [ ] Verify all ban rules are active: `watchclaw status`")
if clusters:
    for c in clusters[:2]:
        body_lines.append(f"- [ ] Approve ASN block for {c['asn']}: `watchclaw enforce ban-asn {c['asn']} --mode ufw`")
body_lines.append(f"- [ ] Review cowrie logs for novel attack patterns")
body_lines.append(f"- [ ] Check geo anomalies in WatchClaw weekly report")
body_lines.append(f"")
body_lines.append(f"---")
body_lines.append(f"*Auto-generated by WatchClaw Security System on {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}*")

result = {
    'title': title,
    'body': '\n'.join(body_lines),
    'labels': ['security', 'critical', 'watchclaw'],
}
...
python3 - "$WATCHCLAW_STATE" "$NOW_ISO" "$GH_URL" <<'PYEOF' 2>/dev/null || true
import sys, json, os
...
if [ -n "$GH_URL" ]; then
    MSG="🚨 WatchClaw CRITICAL ALERT

GitHub issue created: $GH_URL
Severity: $SEVERITY
Time: $NOW_ISO

Run \`watchclaw status\` for full details."
...
echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] watchclaw-critical-issue: issued title='$ISSUE_TITLE' url='$GH_URL'" >> "$LOG"
