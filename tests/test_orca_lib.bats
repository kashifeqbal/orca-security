#!/usr/bin/env bats
# =============================================================================
# tests/test_orca_lib.bats — Tests for lib/orca-lib.sh
# =============================================================================
# Covers:
#   - orca_init: directory and file creation
#   - orca_record_event: per-event-type score deltas
#   - orca_check_and_ban: ban tier thresholds
#   - Instant ban on login_success (honeypot)
#   - Recon cap: max 30 pts per IP per 30m
#   - Score decay via orca_decay_all (10%/24h)
#   - orca_get_score
#   - orca_prune_db
#   - orca_rolling_score
# =============================================================================
...
@test "orca_init creates ORCA_DIR" {
    rm -rf "${ORCA_DIR}"
    orca_init
    [ -d "${ORCA_DIR}" ]
}

@test "orca_init creates threat-db.json" {
    [ -f "${ORCA_DB}" ]
    run jq '.' "${ORCA_DB}"
    [ "$status" -eq 0 ]
}

@test "orca_init creates reputation-cache.json" {
    [ -f "${ORCA_REP_CACHE}" ]
}

@test "orca_init creates asn-db.json" {
    [ -f "${ORCA_ASN_DB}" ]
}

@test "orca_init creates geo-db.json with expected shape" {
    [ -f "${ORCA_GEO_DB}" ]
    run jq '.countries' "${ORCA_GEO_DB}"
    [ "$status" -eq 0 ]
}

@test "orca_init creates orca-state.json with alert_rates key" {
    [ -f "${ORCA_STATE}" ]
    run jq '.alert_rates' "${ORCA_STATE}"
    [ "$status" -eq 0 ]
}

@test "orca_init is idempotent (safe to call multiple times)" {
    orca_init
    orca_init
    run jq '.' "${ORCA_DB}"
    [ "$status" -eq 0 ]
...
@test "failed_login adds 1 point" {
    run orca_record_event "1.2.3.4" "failed_login"
    [ "$status" -eq 0 ]
...
@test "login_success adds 5 points" {
    run orca_record_event "1.2.3.5" "login_success"
    [ "$status" -eq 0 ]
...
@test "command_exec adds 5 points" {
    run orca_record_event "1.2.3.6" "command_exec"
    [ "$status" -eq 0 ]
...
@test "recon_fingerprint adds 3 points" {
    run orca_record_event "1.2.3.7" "recon_fingerprint"
    [ "$status" -eq 0 ]
...
@test "tunnel_tcpip adds 20 points" {
    run orca_record_event "1.2.3.8" "tunnel_tcpip"
    [ "$status" -eq 0 ]
...
@test "persistence_attempt adds 50 points" {
    run orca_record_event "1.2.3.9" "persistence_attempt"
    [ "$status" -eq 0 ]
...
@test "malware_download adds 75 points" {
    run orca_record_event "1.2.3.10" "malware_download"
    [ "$status" -eq 0 ]
...
@test "multiple events accumulate score" {
    orca_record_event "10.0.0.1" "failed_login"   # +1
    orca_record_event "10.0.0.1" "failed_login"   # +1
    orca_record_event "10.0.0.1" "failed_login"   # +1
...
@test "unknown event type defaults to 1 point" {
    run orca_record_event "10.0.0.2" "unknown_event_xyz"
    [ "$status" -eq 0 ]
...
@test "event_types counter is updated" {
    orca_record_event "10.0.0.3" "failed_login"
    orca_record_event "10.0.0.3" "failed_login"
...
@test "score < 25 results in no ban" {
    ...
    orca_record_event "20.0.0.1" "failed_login"
    orca_record_event "20.0.0.1" "failed_login"
    orca_record_event "20.0.0.1" "failed_login"
    run orca_check_and_ban "20.0.0.1"
    [ "$status" -eq 0 ]
    [ "$output" = "none" ]
}
...
@test "score >= 25 triggers short ban" {
    ...
    for i in $(seq 1 5); do
        orca_record_event "20.0.0.2" "command_exec"
    done
    run orca_check_and_ban "20.0.0.2"
    [ "$status" -eq 0 ]
    [ "$output" = "short" ]
}
...
@test "score >= 75 triggers long ban" {
    ...
    orca_record_event "20.0.0.3" "malware_download"
    run orca_check_and_ban "20.0.0.3"
    [ "$status" -eq 0 ]
    [ "$output" = "long" ]
}
...
@test "score >= 150 triggers permanent ban" {
    ...
    orca_record_event "20.0.0.4" "malware_download"
    orca_record_event "20.0.0.4" "malware_download"
    run orca_check_and_ban "20.0.0.4"
    [ "$status" -eq 0 ]
    [ "$output" = "permanent" ]
}
...
@test "ban tier escalates: existing short ban upgraded to long" {
    ...
    # This bypasses orca_record_event so we avoid the double-penalty-during-active-ban
    ...
    run orca_check_and_ban "20.0.0.5"
    [ "$status" -eq 0 ]
    [ "$output" = "long" ]
}

@test "orca_check_and_ban returns none for unknown IP" {
    run orca_check_and_ban "99.99.99.99"
    [ "$status" -eq 0 ]
    [ "$output" = "none" ]
}

@test "active_ban field is set to true in DB after ban" {
    orca_record_event "20.0.0.6" "malware_download"
    orca_check_and_ban "20.0.0.6"
...
@test "login_success triggers instant short ban regardless of score" {
    ...
    orca_record_event "30.0.0.1" "login_success"
    run orca_check_and_ban "30.0.0.1"
    [ "$status" -eq 0 ]
...
@test "login_success + high score escalates to long ban" {
    ...
    orca_record_event "30.0.0.2" "login_success"
    orca_record_event "30.0.0.2" "malware_download"
    run orca_check_and_ban "30.0.0.2"
    [ "$status" -eq 0 ]
    [ "$output" = "long" ]
}
...
@test "login_success + very high score escalates to permanent ban" {
    ...
    orca_record_event "30.0.0.3" "login_success"
    orca_record_event "30.0.0.3" "malware_download"
    orca_record_event "30.0.0.3" "malware_download"
    run orca_check_and_ban "30.0.0.3"
    [ "$status" -eq 0 ]
    [ "$output" = "permanent" ]
}

@test "multiple login_success events are tracked in event_types" {
    orca_record_event "30.0.0.4" "login_success"
    orca_record_event "30.0.0.4" "login_success"
...
@test "recon_fingerprint score is capped at 30 pts per IP in 30m window" {
    ...
    for i in $(seq 1 11); do
        orca_record_event "40.0.0.1" "recon_fingerprint"
    done
...
@test "recon cap is exactly 30 points (10 events x 3pts)" {
    for i in $(seq 1 10); do
        orca_record_event "40.0.0.2" "recon_fingerprint"
    done
...
@test "recon cap does not affect non-recon events" {
    # Fill up recon cap
    for i in $(seq 1 11); do
        orca_record_event "40.0.0.3" "recon_fingerprint"
    done
    # Now add a tunnel event — should not be capped
    orca_record_event "40.0.0.3" "tunnel_tcpip"
...
@test "11th recon event adds 0 pts when cap reached" {
    ...
    for i in $(seq 1 10); do
        orca_record_event "40.0.0.4" "recon_fingerprint"
    done
...
@test "orca_decay_all reduces scores after 24h" {
    orca_record_event "50.0.0.1" "malware_download"  # 75 pts
...
@test "orca_decay_all does not decay score unseen < 24h" {
    orca_record_event "50.0.0.2" "malware_download"  # 75 pts
...
@test "orca_decay_all returns count of IPs decayed" {
    orca_record_event "50.0.0.3" "failed_login"
    db_set_last_seen_hours_ago "50.0.0.3" 25
    orca_record_event "50.0.0.4" "failed_login"
...
@test "score decays to 0 after many days, not negative" {
    orca_record_event "50.0.0.5" "failed_login"  # 1 pt
...
@test "decay during orca_record_event applies automatically" {
    orca_record_event "50.0.0.6" "malware_download"  # 75 pts
    db_set_last_seen_hours_ago "50.0.0.6" 48
    # Record another event — this should apply 2-day decay first, then add delta
    orca_record_event "50.0.0.6" "failed_login"  # +1 after decay
...
@test "orca_get_score returns correct score after event" {
    orca_record_event "60.0.0.1" "tunnel_tcpip"
...
@test "orca_prune_db removes IPs unseen > 45 days" {
    orca_record_event "70.0.0.1" "failed_login"
...
@test "orca_prune_db keeps IPs seen within 45 days" {
    orca_record_event "70.0.0.2" "malware_download"
...
@test "orca_prune_db returns count of removed IPs" {
    orca_record_event "70.0.0.3" "failed_login"
...
@test "orca_rolling_score includes recent event deltas" {
    orca_record_event "80.0.0.1" "tunnel_tcpip"     # 20 pts
    orca_record_event "80.0.0.1" "failed_login"     # 1 pt
    run orca_rolling_score 30
    [ "$status" -eq 0 ]
...
@test "orca_rolling_score default window is 30 minutes" {
    orca_record_event "80.0.0.2" "command_exec"
...
@test "events during an active ban double the score delta" {
    ...
    for i in $(seq 1 5); do
        orca_record_event "90.0.0.1" "command_exec"
    done
    orca_check_and_ban "90.0.0.1"

    score_before=$(db_score "90.0.0.1")

    # Add one more failed_login — normally +1, but should be +2 during active ban
    orca_record_event "90.0.0.1" "failed_login"
